...
- 简单工厂模式不直接向客户端(高层)暴露对象创建的细节，而是通过一个工厂类来负责创建产品类的实例。
- 简单工程模式的角色有：
  * 工厂角色(Createor)
  * 抽象产品角色(Product)
...
- 优点
  * 工厂方法模式的每个具体产品对应一个具体的工厂类, 不需要修改工厂类代码
  * 能满足隐藏对象创建的细节
- 缺点
  * 每增加一个具体产品类，就必须增加一个相应的具体方法

通过例子来加深理解，每个具体产品对应一个具体的工厂类：
```py
from abc import ABCMeta, abstractmethod
...
- 相比工厂方法模式，`抽象工厂模式中的每一个具体工厂都生产一套产品, 而不是一个产品`
- 抽象工厂模式的角色有：
  * 抽象工厂角色(Creator)
  * 具体工厂角色(Concrete Creator)
  * 抽象产品角色(Product)
...
- 缺点
  * 难以支持新种类(抽象)产品, 例如像下面的例子，如果还想增加其他的和手机壳，CPU，操作系统相似的类对象则需要大改
...
写个例子来加强理解：Player就是需要创建的对象，通过Director来控制创建对象Player的顺序, 这样更换对象只需要更换Player，更换顺序就调整Director
```py
from abc import ABCMeta, abstractmethod
...
- 优点
  * 对唯一实例的受控访问（只有一个实例）
  * 单例相当于全局变量，但防止了命名空间被污染（变量命名不会有冲突）
...
- 如果实例只出现一次，如日志系统中只需要创建一个日志对象（否则两个日志对象同时操作一个文件就会造成操作冲突）
